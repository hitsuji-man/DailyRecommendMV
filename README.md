...
- **ゲストから会員へ自然に移行できる認証設計**  
  端末UUIDを利用した匿名ログインに対応。会員登録時は匿名ユーザーを正規ユーザーへ昇格させ、同じユーザーデータを引き継ぎます。
...
| 認証 | 匿名ログイン、会員登録、ログイン、ログアウト、プロフィール・パスワード変更 |
...
### 4. 匿名体験から会員登録までを分断しない

初回利用時は端末UUIDを発行し、ゲストとして即時利用できます。その後の会員登録では新規ユーザーを作り直さず、匿名ユーザーを正規ユーザーへ昇格。認証トークンも端末単位で管理しています。
...
画面構成の詳細は[画面遷移図](documents/画面遷移図.png)、仕様は[要件定義書](documents/要件定義書.md)をご覧ください。
...
| Authentication | Bearer Token、端末UUIDによる匿名認証 |
